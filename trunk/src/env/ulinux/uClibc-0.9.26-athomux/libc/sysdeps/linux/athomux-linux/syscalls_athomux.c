...
//#define __NR_exit             1 
#ifdef L___syscall_exit
#define __NR___syscall_exit __NR_exit 
static inline 
_syscall1(void, __syscall_exit, int, status);

void _exit(int status)
{
	__syscall_exit(status);
	INLINE_SYSCALL_LINUX(exit, 1, 0);
}
#endif

//#define __NR_fork             2
#ifdef L___syscall_fork
#include <unistd.h>
#define __NR___syscall_fork __NR_fork
_syscall3(pid_t, __syscall_fork, unsigned, bp, unsigned, sp, unsigned, ip);
pid_t __libc_fork() {
	...
	return __syscall_fork(bp, sp, ip);
}
...
//#define __NR_read             3
#ifdef L___syscall_read
#include <unistd.h>
#define __NR___syscall_read __NR_read
static inline 
_syscall3_buf(ssize_t, __syscall_read, int, fd, __ptr_t, buf, size_t, count);
ssize_t __libc_read(int fd, __ptr_t buf, size_t count) {

  __syscall_read(fd, buf, count);
  RECEIVE_BUF(buf);
...
//#define __NR_write            4 
#ifdef L___syscall_write
#include <unistd.h>
#define __NR___syscall_write __NR_write
static inline 
_syscall3_buf(ssize_t, __syscall_write, int, fd, const __ptr_t, buf, size_t, count);
ssize_t __libc_write(int fd, const __ptr_t buf, size_t count) {

  __syscall_write(fd, buf, count);
  SEND_BUF(buf, count);
  return RECEIVE_RESULT;
}
...
#include <sys/param.h>
static inline
_syscall3_buf(int, __syscall_open, const char *, file, int, flags, __kernel_mode_t, mode);
int __libc_open (const char * file, int flags, ...)
{
	...
	len = strlen(file);
	__syscall_open(file, flags, mode);
	SEND_BUF(file, len + 1);
	return RECEIVE_RESULT;
}
...
#define __NR___syscall_unlink __NR_unlink
static inline _syscall1_buf(int, __syscall_unlink, const char *, pathname);

int __libc_unlink(const char *pathname) {

  __syscall_unlink(pathname);
  SEND_BUF(pathname, strlen(pathname) + 1);
  return RECEIVE_RESULT;
}
weak_alias(__libc_unlink, unlink);
#endif
...
#include <sys/param.h>
static inline
_syscall3_buf(int, __syscall_execve, const char *, filename, 
...
int __libc_execve(const char *filename, char *const *argv, char *const *envp) {
  ...
  // FIXME: send envp
  __syscall_execve(filename, argv, envp);
  SEND_BUF(filename, strlen(filename) + 1);
  SEND_BUF(&argc, sizeof(int));
  for (index = 0; index < argc; index ++)
    SEND_BUF(argv[index], strlen(argv[index]) + 1);

  int res = RECEIVE_RESULT;
...
weak_alias(__libc_execve, execve)
...
//#define __NR_getuid           24  _syscall0(uid_t, __syscall_getuid);
...
uid_t getuid(void)
{
  return(INLINE_SYSCALL(__syscall_getuid, 0));
}
#endif
...
//#define __NR_utime            30 _syscall2(int, utime, const char *, file, struct utimbuf *, times);
#ifdef L_utime
#include <utime.h>
#define __NR___syscall_utime __NR_utime
static inline _syscall2_buf(int, __syscall_utime, const char *, file, const struct utimbuf *, times);

int utime(const char *file, const struct utimbuf *times) {

  __syscall_utime(file, times);
  int len = strlen(file) + 1;
  SEND_BUF(file, len);
  if (!times) {
    struct utimbuf b = {42, 42}; ///gettimeofday
    SEND_BUF(&b, sizeof(struct utimbuf)); 
  }

  else 
    SEND_BUF(times, sizeof(struct utimbuf));
...
#define __NR___syscall_access __NR_access
static inline _syscall2_buf(int, __syscall_access, const char *, pathname, int, mode);

int __libc_access(const char *pathname, int mode) {

  __syscall_access(pathname, mode);
  SEND_BUF(pathname, strlen(pathname) + 1);
  return RECEIVE_RESULT;
}
weak_alias(__libc_access, access);
...
//#define __NR_pipe             42
#ifdef L___syscall_pipe
#define __NR___syscall_pipe __NR_pipe
#include <unistd.h>
_syscall1_buf(int, __syscall_pipe, int *, filedes);

int pipe(int * filedes) {

  __syscall_pipe(filedes);
  SEND_BUF(filedes, 2 * sizeof (int));
  RECEIVE_BUF(filedes);
  return RECEIVE_RESULT;
}
#endif
...
#define __NR___syscall_gettimeofday     78
#ifdef L___syscall_gettimeofday
#include <sys/time.h>
_syscall2_buf(int, __syscall_gettimeofday, struct timeval *, tv, struct timezone *, tz);

int __libc_gettimeofday(struct timeval *tv, struct timezone *tz) {
	__syscall_gettimeofday(tv, tz);
	RECEIVE_BUF(tv);
	return RECEIVE_RESULT;
}
...
#include <bits/stat.h>
#include "xstatconv.h"
_syscall2_buf(int, __syscall_stat, const char *, file_name, struct stat *, buf);
int stat(const char * file_name, struct stat * buf)
{
	int len;
	__syscall_stat(file_name, buf);
	len = strlen(file_name) + 1;
	SEND_BUF(file_name, len);
	RECEIVE_BUF(buf);
	return RECEIVE_RESULT;
}
...
#include "xstatconv.h"
_syscall2_buf(int, __syscall_lstat, const char *, file_name, struct stat *, buf);
int lstat(const char * file_name, struct stat * buf)
{
	int len;
	__syscall_lstat(file_name, buf);
	len = strlen(file_name) + 1;
	SEND_BUF(file_name, len);
	RECEIVE_BUF(buf);
	return RECEIVE_RESULT;
...
//#define __NR_uname            122 _syscall1(int, uname, struct utsname *, buf);
#ifdef L_uname
#include <sys/utsname.h>
#define __NR___syscall_uname __NR_uname
static inline _syscall1_buf(int, __syscall_uname, struct utsname *, buf);

int __libc_uname(struct utsname * buf) {
...
#ifdef L___syscall_getdents
#define __NR___syscall_getdents __NR_getdents
_syscall3_buf(int, __syscall_getdents, int, fd, char *, kdirp, size_t, count);
int __getdents(int fd, char *buf, size_t nbytes)
{
  __syscall_getdents(fd, buf, nbytes);
  RECEIVE_BUF(buf);
  return RECEIVE_RESULT;
}
...
#ifdef L___syscall_nanosleep
#include <time.h>
#define __NR___syscall_nanosleep __NR_nanosleep
_syscall2_buf(int, __syscall_nanosleep, const struct timespec *, req, struct timespec *, rem);

int __libc_nanosleep(const struct timespec * req, struct timespec * rem) {
  __syscall_nanosleep(req, rem);
  SEND_BUF(req, sizeof(struct timespec));
  SEND_BUF(rem, sizeof(struct timespec));
  return RECEIVE_RESULT;
}
...
#ifdef L___libc_getcwd
#define __NR___libc_getcwd __NR_getcwd
static inline 
_syscall2_buf(int, __libc_getcwd, char *, buf, unsigned long, size);

int __syscall_getcwd(char *buf, unsigned long size) {

  __libc_getcwd(buf, size);
  RECEIVE_BUF(buf);
  int r = RECEIVE_RESULT;
  if (r != 0)
    return buf;
  else
    return 0;
...
char *getcwd(char *buf, size_t size) {
    ...
    int ret = __syscall_getcwd(buf, (int) size);
    return buf;
}
#endif
...
#ifdef L___syscall_vfork
#include <unistd.h>
#define __NR___syscall_vfork __NR_vfork
_syscall3(pid_t, __syscall_vfork, unsigned, bp, unsigned, sp, unsigned, ip);
...
pid_t __libc_vfork() {
	...
	printf("__libc_vfork(): eip: 0x%08x\n", ip);

	SYNC();

	return __syscall_vfork(bp, sp, ip);
	/*
	printf("pid: %d\n", pid);
